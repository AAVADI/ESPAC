...
- `outputs/`: runtime-generated SQLite, CSV, spreadsheet, cache, report, and XML artifacts; this directory is intentionally not versioned, except for a placeholder `.gitkeep`.
...
- treat everything under `outputs/` as rebuildable local state, not as repository source data.
...
## 6. Core outputs generated by the pipeline

`outputs/` is no longer committed to Git. The files listed in this section are the canonical runtime artifacts produced locally by the notebooks, scripts, or marimo app. To obtain them, rerun the relevant pipeline stages.
...
Crop filtered outputs currently include:
...
Crop DFE outputs currently include:
...
Livestock outputs currently include summary tables for:
...
Livestock DFE outputs currently include:
...
The project now supports two optional expensive-build reference caches used by the marimo app:
...
Additional synthetic or locked diagnostic exports may exist locally, but they are disposable runtime artifacts unless a specific reproducibility need justifies versioning them elsewhere.

### XML outputs

Crop XML outputs are written locally to:
...
Livestock XML outputs are written locally to:
