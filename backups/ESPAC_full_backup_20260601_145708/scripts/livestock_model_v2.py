...
def build_product_lci_v2(stock: pd.DataFrame, emissions: pd.DataFrame, con: sqlite3.Connection) -> pd.DataFrame:
    gl = pd.read_sql_query(
        "SELECT identificador, ual_prov, gl_litlecvacaje, gl_litlecven, vacas_ordenadas, litros_orde_ados, gl_vacas_ta, fact_exp_fin "
        "FROM rel_inec_glnac",
        con,
    )
    ap = pd.read_sql_query(
        "SELECT identificador, ual_prov, ap_venta_hcodor, ap_prod_hcodor, avesp_ventas, ap_ctponedoras, fact_exp_fin, "
...
def main() -> None:
    ...
    parser.add_argument(
        "--v1-reference",
        default="outputs/CSVs/03-05_espac_livestock_lci_table_filtered_dfe__summary_product.csv",
        help="V1 summary-product CSV used to calibrate V2 magnitudes",
    )
    parser.add_argument(
